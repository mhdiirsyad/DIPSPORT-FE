...
const filteredFields = computed(() => {
  ...
  return fields.value.filter((field) =>
    ...
    (field.Stadion && field.Stadion.name.toLowerCase().includes(query)) ||
...
const itemsPerPage = 10 // Anda bisa sesuaikan ini
...
async function handleDelete(id: number) {
  if (confirm('Anda yakin ingin menghapus lapangan ini?')) {
    ...
    try {
      ...
      // DITAMBAHKAN: Cek jika halaman jadi kosong setelah hapus
  ...
  <section class="flex w-full flex-col gap-7">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div class="border-b border-gray-200 p-5 sm:p-6">
        <label class="relative flex items-center gap-2.5 rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2.5">
          ...
          <input
            ...
            placeholder="Cari berdasarkan nama lapangan, atau stadion..."
          />
        </label>
      </div>
      ...
      <div class="flex flex-col">
        ...
        <div v-else-if="filteredFields.length === 0" class="px-5 sm:px-6 py-10 text-center text-sm text-gray-500">
          <span v-if="searchQuery">Lapangan tidak ditemukan.</span>
          <span v-else>Belum ada data lapangan.</span>
        </div>

        <!-- INI ADALAH PERBAIKANNYA: "overflow-x-auto" -->
          ...
          <table class="min-w-full border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
              ...
              >
                ...
                <td 
                  class="whitespace-nowrap px-5 py-4 text-sm text-gray-900"
                  @click="navigateTo(`/admin/fields/${field.id}`)"
                >
